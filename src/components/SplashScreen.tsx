...
const SplashScreen = ({ onAnimationComplete }: SplashScreenProps): ReactNode => {
  ...
  return (
    <main className='grid h-screen w-3/4 grid-cols-1 grid-rows-[2fr,1fr,1fr] items-center justify-items-center font-pretendard'>
      <motion.div
      ...
      {/* Fade-in 문구 */}
      <Motion tag='p' animation={fadeIn(-40, 0, 1, 2, true)} className='mb-6 self-end text-lg font-bold'>
        프론트엔드 개발자 김지호 포트폴리오
      </Motion>
      <Motion
        tag='p'
        animation={fadeIn(-40, 0, 1, 3, true)}
